...
tag: ['Vim']
cover: ''
description: '这篇文章分享了我自定义的 Vim 键位，我在 VSCode、NeoVim、IDEA 中使用这套键位开发了很多年。同时介绍了多编辑器同步 Vim 快捷键的方法'
---
...
## 多编辑器同步键位
我常用的编辑器是 VSCode、NeoVim、IDEA系（RubyMine、WebStorm）， 针对不同场景因地制宜的使用不同的编辑器。  
...
### IDEA 系
IDEA 系列编辑器自带 Vim 插件，其配置文件位于 `~/.config/.ideavimrc`。  
对应的配置文件如下：
```vim
" 基础设置
...
" 输入模式键位配置
imap jj <Esc>        " 使用 jj 快速退出输入模式

" 命令模式键位配置
nnoremap sa :w<CR>                " 保存文件
nnoremap wq :wq<CR>              " 保存并关闭文件
nnoremap qf :action QuickFix<CR>  " 显示快速修复建议
nnoremap gd :action GotoDeclaration<CR>    " 跳转到定义
nnoremap gr :action FindUsages<CR>         " 查看引用
nnoremap gs :action FindInPath<CR>         " 全局搜索
nnoremap cs :action Find<CR>               " 当前文件内搜索
nnoremap x :q<CR>                         " 关闭当前文件

" 移动相关键位配置
nnoremap <Space>k 10k   " 向上移动 10 行
nnoremap <Space>j 10j   " 向下移动 10 行
nnoremap <Space>h 10h   " 向左移动 10 个字符
nnoremap <Space>l 10l   " 向右移动 10 个字符

" 快速移动键位（使用 AceJump 插件）
nnoremap <Space><Space>j :action AceAction<CR>                " 跳转到任意位置
nnoremap <Space><Space>k :action AceLineAction<CR>           " 跳转到行首
nnoremap <Space><Space>h :action AceWordBackwardAction<CR>   " 向前跳转到单词
nnoremap <Space><Space>l :action AceWordAction<CR>           " 向后跳转到单词

" 分屏操作键位
nnoremap <Space>sx :action SplitHorizontally<CR>    " 水平分屏
nnoremap <Space>sy :action SplitVertically<CR>      " 垂直分屏
nnoremap <Space>sc :action CloseContent<CR>         " 关闭当前分屏

" 窗口切换键位
nnoremap <Space>sh :action PreviousWindow<CR>       " 切换到左边窗口
nnoremap <Space>sl :action NextWindow<CR>           " 切换到右边窗口
nnoremap <Space>sk :action PreviousWindow<CR>       " 切换到上边窗口
nnoremap <Space>sj :action NextWindow<CR>           " 切换到下边窗口

" 启用一些 IdeaVim 插件
set easymotion           " 启用 EasyMotion 插件
set surround             " 启用 surround 插件
set multiple-cursors     " 启用多光标支持
set commentary           " 启用注释插件
```