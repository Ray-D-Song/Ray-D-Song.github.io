...
## 什么是事件循环
事件循环 的概念非常简单。它是一个在 JavaScript 引擎`等待任务`，`执行任务`和`进入休眠状态等待更多任务`这几个状态之间转换的无限循环。  
...
## 为什么需要事件循环
事件循环并不是 JavaScript 独有的模型，比如 Python 的 [asyncio](https://docs.python.org/3/library/asyncio.html)
事实上，单线程的应用如果想处理外部事件和耗时的 IO 处理，都离不开消息循环。

事件循环通常绑定的关键词是「异步操作」，要理解为什么需要事件循环，可以反过来假设如果没有事件循环的情况。  
